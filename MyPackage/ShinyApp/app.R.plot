...
tags$div(
   ...
   tabPanel("Plots",  icon=icon("question-circle-o", class="fa fa-question-circle-o fa-lg  "),
                fluidPage(
...
#})
  ...
  ## gets number of iterations performed of AM
  itnum <- reactive({
    if(input$analyse_go){
       if(length(res$Mrk)==1 & is.na(res$Mrk[1])) 
       {
           return(NA)
       } else {
          # number of iterations performed = number of markers found
          return(length(res$Mrk[-1]))
       }
 ...
 # sets up iteratin number for plotting 
  sze <- 0
  ...
  selectInput(inputId="plotnumber", label=h4("Choose iteration number of model building process whose data you wish to plot.  "), choices=nits  , size = sze   , selectize=FALSE )
  })  ## end renderUI
