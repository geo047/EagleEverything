     ...
     for(long i=0; i <  n_of_cols_in_geno; i++){
        ...
        if( rowvec[ (2*i ) ] == '0' ||  rowvec[ (2*i + 1) ] == '0' || rowvec[ (2*i ) ] == '-' ||  rowvec[ (2*i + 1) ] == '-'){
           if (printOnlyOnce == 0){
                 ...
                 message("           These missing genotypes should be imputed before running Lion." );
                 message("           As an approximation, AMpus has set these missing genotypes to heterozygotes. " );
                 message("           Since Lion assumes an additive model, heterozygote genotypes do not contribute to the estimation of " );
