...
#' @details
#' 
#' \code{ReadVCF} can handle two different types of marker data; namely,
...
#' To load a text file that contains snp genotypes, run \code{ReadVCF} with \code{filename} set to the name of the file, 
...
#'
#' \preformatted{geno_obj <- ReadVCF(filename='/my/dir/geno.txt', AA=0, AB=1, BB=2, type='text', missing=99)}
#'
...
#' To load these data, we would use the command 
#'
#' \preformatted{geno_obj <- ReadVCF(filename='geno.txt', AA='a/a', AB='a/b', BB='b/b', 
...
#'
#'
#' Here, these columns can be any values since \code{ReadVCF} ignores these columns.  
#'
...
#'
#' \preformatted{geno_obj <- ReadVCF(filename='PLINK.ped', type='PLINK')}
#'
...
#'
#'
#' @return  To allow \code{\link{AM}} to handle data larger than the memory capacity of a machine, \code{ReadVCF} doesn't load 
...
#' \code{ReadVCF} is a list object with the elements \code{asciifileM} , \code{asciifileMt}, and \code{dim_of_ascii_M}  
...
ReadVCF <- function( filename=NULL, availmemGb=16, quiet=TRUE ){
 ...
 geno <- list("asciifileM"=binfileM, "asciifileMt"=binfileMt,
               "dim_of_ascii_M" = dim_of_M,
               "dim_of_ascii_Mt" = c(dim_of_M[2], dim_of_M[1]),
