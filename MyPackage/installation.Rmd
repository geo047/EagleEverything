...
1. Can install Lion_1.0.0.tar.gz using R CMD INSTALL Lion_1.0.0.tar.gz
   but need to install dependent packages Rcpp RcppEigen shiny first. 
2. Go into R. 
   use install.packages("Lion_1.0.0.tar.gz", repos=NULL)   
   but you also have to install dependent packages first. 
3. From CRAN, you can use 
   install.packages("Lion", dependencies=TRUE)  and this will install all dependent packages 
   as well as Lion. 
...
then typed
install.packages("Lion_1.0.0.0.tar.gz", repos=NULL)
...
Notes - Ubuntu 
29/03/17
R CMD INSTALL Lion_1.0.0.tar.gz on virgin R system failed due to dependencies 
