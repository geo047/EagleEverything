 ...
 *      Author: Josh Bowden
 *      CSIRO
 *      class CBasicFunctions:  A class that has various standard functions. Manipulates input strings
 *      from an input file of general format "xyz = abc" Has directory Creation and file manipulation
 *      methods. Has a number of string manipulation functions (based on pascal/delphi)
 *
 */
...
public:
    ...
    size_t  pos				 ( char*   subStr, string  STR) ;		// delphi sysutils styled function - returns the "1 based" position of the string (also returns 1 if subStr is an empty string)
    size_t  pos				 ( string  subStr, string  STR) ;		// returns the "1 based" position of the string (also returns 1 if subStr is an empty string)
    string  copy			 ( string STR, long long start,size_t number) ;// delphi styled function
    int     length( string   tStr ) ;
    ...
    size_t		CBasicFunctions::pos( char*   subStr, string  STR)
    {
    ...
    // i.e same as delphi pos(substr,string) function
    // not the "0 based" C variety
    size_t		CBasicFunctions::pos( string  subStr, string  STR) // returns the "1 based" position of the string (also returns 1 if subStr is an empty string)
    {
        size_t retVal = STR.find(subStr) ;
        retVal += 1 ; // (as delphi version returns 0 if subStr does not exist and not -1 like C++ .find() function)
        return retVal ;	
    ...
    // finds last 
    size_t		CBasicFunctions::poslast( string  subStr, string  STR) // returns the "1 based" position of the string (also returns 1 if subStr is an empty string)
    {
    ...
    string	CBasicFunctions::copy	( string STR, long long start,size_t number)
    {
        string resStr = "" ;
        if (number > 0)
        {
    ...
    int		CBasicFunctions::length( string   tStr )
    {
        int lengthStr = 0 ;
...
#endif  // !defined(CBasicFunctions_)
...
// This is the preferred data alignment of the machine being compiled on
// it is needed so that strings of odd sizes can be handled when compound
// datatypes are read from disk to memory. On the transfer, the strings 
// buffered to end on a integral value of BYTEALLIGNMENT
#define BYTEALLIGNMENT 8
...
bool CHDF5File<T>::CreateFullHDFGroupPath(  string datasetPathAndNameIn, bool IsPathOnlyIn )
{
    ...
    t_pos = bb.pos((string)"/", t_sRemainingDatasetPath) ;
    if (t_pos == 1)// remove "/" if it is in the first position
    {
        t_sRemainingDatasetPath  = bb.copy(t_sRemainingDatasetPath, 2 , bb.length(t_sRemainingDatasetPath) - t_pos) ;
        t_pos = bb.pos((string)"/", t_sRemainingDatasetPath) ;
        t_sCurrentGroupFullPath  = "/" + bb.copy(t_sRemainingDatasetPath,1,t_pos-1) ;
        t_sRemainingDatasetPath  = bb.copy(t_sRemainingDatasetPath,t_pos+1 , bb.length(t_sRemainingDatasetPath) - t_pos) ;

    }
    else
    {
        t_sCurrentGroupFullPath  = "/" + bb.copy(t_sRemainingDatasetPath,1,t_pos) ;
        t_sRemainingDatasetPath  = bb.copy(t_sRemainingDatasetPath,t_pos +  1, bb.length(t_sRemainingDatasetPath) - t_pos) ;
    }
...
//		H5T_class_t dataset_class	;
    ...
    //		dataset_class					=  H5Tget_class( this->hid_t_MemAttr ) ;
        ...
        //	ret	  = H5Soffset_simple(fileSpace_id, offset) ;
            ...
            ret = H5Dread (t_dataset ,this->hid_t_MemAttr	, memSpace_id, fileSpace_id, plist_id, (void *) memStreamIn.ReturnMem() );
